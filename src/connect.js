  ...
  getQrFirst() {
    return new Promise((resolve, reject) => {
      this._qrCodeLogin().then(async(res) => {
        ...
        let icH = await this._channel.issueChannelToken("1341209950");
        config.chanToken = icH.channelAccessToken;
        let xxc = icH.expiration;
        let xcc = xxc.toString().split(" ");
        let xc = xcc.toString();
        ...
        //   console.info(`Regrads Alfathdirk \n`);
        console.info(`=======LINE AlphatJS======\n`);
        resolve();
      });
    });
  }
